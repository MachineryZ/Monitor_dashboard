...
# 图表用交易时段：夜盘必须写成 21:00→02:30 + crosses_midnight=True
# 不要拆成 (00:00, 02:30, True)，否则 t>=00:00 恒成立，03:00-08:59 会被当成交易时间
CHART_SESSIONS = [
    (datetime.time(21, 0), datetime.time(2, 30), True),
    (datetime.time(9, 0),  datetime.time(10, 15), False),
    (datetime.time(10, 30), datetime.time(11, 30), False),
...
# 两次快照间隔超过该交易分钟数则断线，避免休市/缺文件被画成大斜线
_CHART_MAX_GAP = 15
_POS_SNAP_COLS = [
...
def _break_gaps(df: pd.DataFrame, ycol: str, max_gap: int = _CHART_MAX_GAP) -> pd.DataFrame:
    """时间间隔过大时插入空点，让 Plotly connectgaps=False 断开折线。"""
...
def draw_intraday_charts(
    ...
    # ---- 图2: 合约 Profit / Init Capital（全局，单位bps） ----
    fig2 = go.Figure()
    has_fig2 = False
    ...
    for product_key, (instrument_data, init_cap, broker, product_name) in all_product_data.items():
        ...
        if inst_list:
            product_contract_map[product_key] = inst_list
            for inst, df in instrument_data.items():
                if inst in inst_list:
                    contract_data_map[(product_key, inst)] = df
...
def draw_intraday_charts(
    ...
    for product_key, (instrument_data, init_cap, broker, product_name) in all_product_data.items():
        for inst, df in instrument_data.items():
            if contract_list and inst not in contract_list:
                continue
            display_name = f"{product_key} | {inst}"
            contract_map[display_name] = (product_key, inst, df)
    ...
    if not contract_map:
        ...
        product_key, inst, df = contract_map[selected_display]
        ...
        fig4.update_layout(
            title=f"合约 {inst} (产品 {product_key}) 持仓与盈亏",
            xaxis=xaxis_dict,
            yaxis=dict(
                title="持仓 (手)",
                autorange=True,
